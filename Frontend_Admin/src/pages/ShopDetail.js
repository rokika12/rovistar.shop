...
function OverviewTab({ shop, setExpiry, toggleStatus, onSaved }) {
  ...
  const [appearance, setAppearance] = useState({
    ...
    product_text_color: shop.theme?.appearance?.product_text_color || 'default',
    sale_text_color: shop.theme?.appearance?.sale_text_color || 'default',
    ...
    google_client_id: shop.theme?.appearance?.google_client_id || '',
    section_kicker_color: shop.theme?.appearance?.section_kicker_color || '#b88712',
    section_title_color: shop.theme?.appearance?.section_title_color || '#d62468',
    section_accent_color: shop.theme?.appearance?.section_accent_color || '#1677db',
  });
        ...
        <div className="border-b pb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2 rounded-xl border border-rose-100 bg-rose-50/40 p-4">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">ពណ៌អក្សរជួរទំនិញ<select value={appearance.product_text_color} onChange={(e) => setAppearance({ ...appearance, product_text_color: e.target.value })} className={`${inputCls} mt-1`}><option value="default">ពណ៌តាម theme</option><option value="blue">ខៀវ</option><option value="red">ក្រហម</option><option value="pink">ផ្កាឈូក</option><option value="green">បៃតង</option><option value="black">ខ្មៅ</option></select></label><label className="text-xs font-semibold">ពណ៌អក្សរជួរបញ្ចុះតម្លៃ<select value={appearance.sale_text_color} onChange={(e) => setAppearance({ ...appearance, sale_text_color: e.target.value })} className={`${inputCls} mt-1`}><option value="default">ពណ៌តាម theme</option><option value="blue">ខៀវ</option><option value="red">ក្រហម</option><option value="pink">ផ្កាឈូក</option><option value="green">បៃតង</option><option value="black">ខ្មៅ</option></select></label></div>
              ...
              <div className="mt-3 border-t border-rose-100 pt-3"><label className="block text-xs font-semibold">Google Client ID សម្រាប់ Login Gmail<input value={appearance.google_client_id} onChange={(e) => setAppearance({ ...appearance, google_client_id: e.target.value.trim() })} className={`${inputCls} mt-1`} placeholder="លេខចប់ដោយ .apps.googleusercontent.com" autoCapitalize="none" /></label><p className="mt-1 text-[11px] text-slate-500">ដាក់តែ Client ID ប៉ុណ្ណោះ។ កុំដាក់ Client Secret នៅទីនេះ។</p></div>
              <div className="mt-3 grid gap-3 border-t border-rose-100 pt-3 sm:grid-cols-3"><label className="text-xs font-semibold">ពណ៌ PICKED FOR YOU<input type="color" value={appearance.section_kicker_color} onChange={(e) => setAppearance({ ...appearance, section_kicker_color: e.target.value })} className="mt-1 h-10 w-full cursor-pointer rounded border bg-white p-1" /></label><label className="text-xs font-semibold">ពណ៌ Popular Products<input type="color" value={appearance.section_title_color} onChange={(e) => setAppearance({ ...appearance, section_title_color: e.target.value })} className="mt-1 h-10 w-full cursor-pointer rounded border bg-white p-1" /></label><label className="text-xs font-semibold">ពណ៌បន្ទាត់ខៀវ<input type="color" value={appearance.section_accent_color} onChange={(e) => setAppearance({ ...appearance, section_accent_color: e.target.value })} className="mt-1 h-10 w-full cursor-pointer rounded border bg-white p-1" /></label></div>
            </div>
          </div>
    ...
    <Modal open={modal} title={editing ? `Edit ${editing.name}` : 'Add Product'} onClose={() => setModal(false)}>
      <form onSubmit={submit} className="space-y-4">
        ...
        {manualOnly && <div className="space-y-3 rounded-xl border border-sky-200 bg-sky-50 p-4">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between"><label className="text-xs font-semibold text-slate-600">Package និងតម្លៃ</label><button type="button" className={btnGhost} onClick={() => setForm({ ...form, variations: [...form.variations, { name: '', price: '', image_url: '', discount_percent: '' }] })}>+ បន្ថែម Package</button></div>
            <div className="space-y-3">{form.variations.map((item, index) => <div key={index} className="rounded-lg border border-slate-200 bg-white p-3"><div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_7rem_auto]"><input value={item.name} onChange={(e) => setForm({ ...form, variations: form.variations.map((row, rowIndex) => rowIndex === index ? { ...row, name: e.target.value } : row) })} className={inputCls} placeholder="ឈ្មោះ Package" /><input type="number" min="0" step="0.01" value={item.price} onChange={(e) => setForm({ ...form, variations: form.variations.map((row, rowIndex) => rowIndex === index ? { ...row, price: e.target.value } : row) })} className={inputCls} placeholder="USD" /><button type="button" className="rounded-lg bg-red-100 px-3 py-2 text-xs font-bold text-red-700" onClick={() => setForm({ ...form, variations: form.variations.filter((_, rowIndex) => rowIndex !== index) })}>លុប</button></div><div className="mt-2 flex items-center gap-3"><input type="number" min="0" max="100" value={item.discount_percent || ''} onChange={(e) => setForm({ ...form, variations: form.variations.map((row, rowIndex) => rowIndex === index ? { ...row, discount_percent: e.target.value } : row) })} className="w-28 rounded-lg border px-2 py-1 text-xs" placeholder="Discount %" /><input type="file" accept="image/*" onChange={(e) => uploadVariationImage(index, e.target.files?.[0])} className="max-w-[14rem] text-xs" />{item.image_url && <img src={fullUrl(item.image_url)} alt="Package preview" className="h-12 w-12 rounded-lg object-cover" />}</div></div>)}</div>
          </div>
        </div>}
        <div>
