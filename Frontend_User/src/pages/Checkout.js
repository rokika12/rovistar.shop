          ...
          {currentShopLoggedIn && !freeDigitalOrder && !new URLSearchParams(window.location.search).has('payment') && (
            <div className="mb-5 text-left">
              ...
              <div className="space-y-3">
                <button type="button" onClick={() => setPaymentMethod('khqr')} className={`flex w-full items-center gap-4 rounded-2xl border-2 p-4 text-left transition ${paymentMethod === 'khqr' ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white'}`}><span className="flex h-12 w-20 shrink-0 items-center justify-center gap-1 rounded-xl bg-[#061d3a] px-1 shadow-sm"><img src={ABA_LOGO_URL} alt="ABA" style={{ display: 'block', width: 43, height: 22, objectFit: 'contain' }} /><img src={KHQR_LOGO_URL} alt="KHQR" style={{ display: 'block', width: 19, height: 19, objectFit: 'contain' }} /></span><span><b className="block text-blue-900">ABA KHQR</b><small className="mt-1 block text-blue-600">Scan to pay with any banking app</small></span></button>
                <button type="button" onClick={() => setPaymentMethod('wallet')} className={`flex w-full items-center gap-4 rounded-2xl border-2 p-4 text-left transition ${paymentMethod === 'wallet' ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white'}`}><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-lg font-black text-white shadow-sm">W</span><span><b className="block text-blue-900">Wallet Balance</b><small className="mt-1 block text-blue-600">Pay instantly from your balance · ${Number(walletBalance).toFixed(2)}</small></span></button>
              </div>
            </div>
          )}
